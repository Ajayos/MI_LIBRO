...
exports.editBook = async (data) => {
	try {
		const { id, ISBN, title, author, genre, description, pic, status } = data;
...
exports.approveRequest = async (data) => {
	try {
		...
		book.request = null;
		book.status = "Rented";
		book.rented.id = uuidv4();
		book.rented.name = user.name;
		book.rented.user = user._id;
 ...
 * Reject a request
 *
 * @param {Object} data - Data for Reject a request.
 * @param {String} data.id - ID of the book.
 * @returns {Object} Response indicating the status, error, message, and data of the process.
 */
exports.RejectRequest = async (data) => {
	try {
		...
		return {
			...
			data: { message: "Book request Rejected successfully" },
		};
	} catch (error) {
		return {
			status: 500,
 ...
 * @param {String} data.id - ID of the book.
 * @param {String} data.user - User.
 * @param {String} data.stars - List of stars.
 * @returns {Object} Response indicating the status, error, message, and data of the process.
 */
exports.returnBook = async (data) => {
	try {
		const { id, user, stars } = data;
		...
		// Find the user by ID
		const user_ = await User.findById(book.rented.user);

		// check access
		var uId1 = new ObjectId(user.id);
		if (!user_._id.equals(uId1)) {
			return {
				status: 403,
				error: true,
				message: "Access Denied",
...
exports.AllBooks = async () => {
	try {
		const book = await Book.find()
			.populate("likes", "name pic email")
			.populate("buyers.user", "name pic email")
			.populate("request", "name pic email")
			.populate("comments.user", "name pic email");
...
exports.getLikedBooks = async (data) => {
	const { id } = data;
	try {
		...
		const books = await Book.find({ likes: id })
			.populate("likes", "name pic email")
			.populate("buyers.user", "name pic email")
			.populate("request", "name pic email")
			.populate("comments.user", "name pic email");

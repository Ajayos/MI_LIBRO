...
exports.createAccount = async (data) => {
	try {
		...
		if (pic.startsWith('data:image/') || pic.startsWith('data:video/')) {
			const base64Data = pic.replace(/^data:(image|video)\/(png|jpeg|jpg|gif);base64,/, '');
			const fileExtension = pic.match(/^data:(image|video)\/(png|jpeg|jpg|gif);base64,/)[2];
			const fileName = `${Date.now()}.${fileExtension}`;
			const filePath =  __dirname + `/bin/${fileName}`;
			fs.writeFileSync(filePath, base64Data, 'base64');
			newUser.pic = fs.readFileSync(filePath, 'base64');
		  } else {
			newUser.pic = pic;
		  }
		await newUser.save();
...
exports.editAdminAccount = async (data) => {
	try {
		...
		if (pic.startsWith('data:image/') || pic.startsWith('data:video/')) {
			...
			const filePath =  __dirname + `/bin/${fileName}`;
			fs.writeFileSync(filePath, base64Data, 'base64');
			user.pic = fs.readFileSync(filePath, 'base64');
		  } else {
			user.pic = pic;
		  }
		await user.save();
...
exports.getUserData = async (data) => {
	...
	try {
		...
		const rentedBooks = await Book.find({ "rented.userid": userId });
		const boughtBooks = await Book.find({ "buyers.userid": userId });
